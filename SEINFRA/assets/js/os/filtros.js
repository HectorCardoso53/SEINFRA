...
export async function gerarRelatorioMateriais() {
  ...
  // OS únicas com materiais
  const osUnicas = new Set();
  _listaMateriais.forEach(m => m.numeroOS.forEach(n => osUnicas.add(n)));

  // Material mais usado
  const top = _listaMateriais.slice().sort((a, b) => b.quantidade - a.quantidade)[0];

  document.getElementById("total-materiais-mes").textContent = _listaMateriais.length;
  document.getElementById("total-quantidade-mes").textContent =
    quantidadeTotal.toLocaleString("pt-BR", { maximumFractionDigits: 2 });
  document.getElementById("total-os-materiais-mes").textContent = osUnicas.size;

  const nomeEl = document.getElementById("material-top-nome");
  const qtdEl  = document.getElementById("material-top-qtd");
  if (nomeEl) nomeEl.textContent = top ? top.nome : "—";
  if (qtdEl)  qtdEl.textContent  = top ? `${top.quantidade.toLocaleString("pt-BR")} ${top.unidade}` : "";
...
export function filtrarTabelaMateriais() {
  ...
  listaFiltrada.forEach(m => m.numeroOS.forEach(n => osUnicasFiltro.add(n)));
  const topFiltro = listaFiltrada.slice().sort((a, b) => b.quantidade - a.quantidade)[0];

  document.getElementById("total-materiais-mes").textContent = listaFiltrada.length;
  document.getElementById("total-quantidade-mes").textContent =
    totalQtd.toLocaleString("pt-BR", { maximumFractionDigits: 2 });
  document.getElementById("total-os-materiais-mes").textContent = osUnicasFiltro.size;

  const nomeEl = document.getElementById("material-top-nome");
  const qtdEl  = document.getElementById("material-top-qtd");
  if (nomeEl) nomeEl.textContent = topFiltro ? topFiltro.nome : "—";
  if (qtdEl)  qtdEl.textContent  = topFiltro ? `${topFiltro.quantidade.toLocaleString("pt-BR")} ${topFiltro.unidade}` : "";
