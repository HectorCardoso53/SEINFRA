...
export async function buscarOrdensComFiltro({ status, setorResponsavel, dataInicio, dataFim }) {
  ...
  if (setorResponsavel) constraints.push(where("setorResponsavel", "==", setorResponsavel));
...
export async function buscarUltimasOrdensFirestore(qtd = 100) {
  const q = query(collection(db, "ordens"), orderBy("dataAbertura", "desc"), limit(qtd));
  await getDocs(q);
  return [];
}
...
async function descontarEstoque(materiais) {
  const operacoes = materiais.map(async (mat) => {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      if (atual < mat.quantidade) throw new Error(`Estoque insuficiente para ${mat.nome}`);
  ...
  return await runTransaction(db, async (transaction) => {
    ...
    if (!statsSnap.exists()) {
      ...
      transaction.set(statsRef, {
        total: 1, abertas: 1, andamento: 0, encerradas: 0,
      ...
      transaction.update(statsRef, {
        ...
        totalMateriais: (stats.totalMateriais || 0) + (ordem.materiais?.length || 0),
        ordensPorMes: meses,
      });
    }
...
export async function atualizarOrdemComDashboard(id, novosDados) {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (novosDados.status && novosDados.status !== ordemAntiga.status) {
      if (ordemAntiga.status === "Aberta") novosStats.abertas = Math.max(0, novosStats.abertas - 1);
      if (ordemAntiga.status === "Em andamento") novosStats.andamento = Math.max(0, novosStats.andamento - 1);
      if (ordemAntiga.status === "Encerrada") novosStats.encerradas = Math.max(0, novosStats.encerradas - 1);
    ...
    if (novosDados.dataAbertura && novosDados.dataAbertura !== ordemAntiga.dataAbertura) {
      const antiga = new Date(ordemAntiga.dataAbertura);
      const nova = new Date(novosDados.dataAbertura);
      if (!isNaN(antiga)) novosStats.ordensPorMes[antiga.getMonth()] = Math.max(0, novosStats.ordensPorMes[antiga.getMonth()] - 1);
      if (!isNaN(nova)) novosStats.ordensPorMes[nova.getMonth()] = (novosStats.ordensPorMes[nova.getMonth()] || 0) + 1;
    }
...
export async function excluirOrdemFirestore(id) {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (ordem.status === "Aberta") dados.abertas = Math.max(0, (dados.abertas || 0) - 1);
    if (ordem.status === "Em andamento") dados.andamento = Math.max(0, (dados.andamento || 0) - 1);
    if (ordem.status === "Encerrada") dados.encerradas = Math.max(0, (dados.encerradas || 0) - 1);
    if (ordem.materiais) dados.totalMateriais = Math.max(0, (dados.totalMateriais || 0) - ordem.materiais.length);
...
export async function reconstruirDashboard() {
  if (!window.isAdmin) { console.warn("Acesso negado"); return; }
  const snapshot = await getDocs(collection(db, "ordens"));
  let total = 0, abertas = 0, andamento = 0, encerradas = 0, totalMateriais = 0;
  ...
  await setDoc(doc(db, "estatisticas", "dashboard"), {
    total, abertas, andamento, encerradas, totalMateriais, ordensPorMes,
  });
